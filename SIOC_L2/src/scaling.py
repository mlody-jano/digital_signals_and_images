...
def linear_interpolation(image):
    ...
    if linear_counter < 5:
        ...
        print(f"Scale: {np.pow(1.1, linear_counter + 1):.2f}")
        print(f"Height: {new_h}, Width: {new_w}")

        im = Image.fromarray(new_img.astype(np.uint8))
        im.save(f'images/linear/enlarged Scale {np.pow(1.1, linear_counter + 1):.2f}.jpg')
        ...
        print(f"Scale: {np.pow(1.1, 10 - linear_counter):.2f}")
        print(f"Height: {new_h}, Width: {new_w}")

        im = Image.fromarray(new_img.astype(np.uint8))
        im.save(f'images/linear/shrinked Scale {np.pow(1.1, 10 - linear_counter):.2f}.jpg')
...
def cubic_interpolation(image):
    ...
    if cubic_counter < 5:
        ...
        print(f"Scale: {np.pow(1.1, cubic_counter + 1):.2f}")
        print(f"Height: {new_img.shape[0]}, Width: {new_img.shape[1]}")

        im = Image.fromarray(new_img.astype(np.uint8))
        im.save(f'images/cubic/enlarged Scale {np.pow(1.1, cubic_counter + 1):.2f}.jpg')
        ...
        print(f"Scale: {np.pow(1.1, 10 - cubic_counter):.2f}")
        print(f"Height: {new_img.shape[0]}, Width: {new_img.shape[1]}")

        im = Image.fromarray(new_img.astype(np.uint8))
        im.save(f'images/cubic/shrinked Scale {np.pow(1.1, 10 - cubic_counter):.2f}.jpg')
